...
MAX_GATE_LENGTH_PERCENTAGE = const(90)
...
class EuclideanRhythm(EuclideanRhythmParameters):
    ...
    def update_gate_length_percentage_time_ms(self, clk_mode, time_delay):
        # when using tap mode the time is in ms (so percent)
        # when using clock input, the time is in tenth of ms (so percent divide by 10)
        if clk_mode == LxEuclidConstant.TAP_MODE:
            self.gate_length_percentage_time_ms = time_delay * \
                self.gate_length_percentage // 100
        else:
            self.gate_length_percentage_time_ms = time_delay * \
                self.gate_length_percentage // 1000
...
class LxEuclidConfig:
    ...
    def __init__(self, lx_hardware, LCD, software_version):
        ...
        self.lx_hardware.set_lx_euclid_config(self)
    ...
    def delegate_load_preset(self):
        ...
        if self.lx_hardware.lx_pander_seq is not None:
            ...
            for index, euclidean_rhythm in enumerate(self.euclidean_rhythms):
                ...
                if euclidean_rhythm.algo_custom:
                    ...
                    self.lx_hardware.set_expander_rhythm(
                        rhythm_copy, index, euclidean_rhythm.beats, euclidean_rhythm.pulses)
                    if not rhythm_found:
            ...
            for index, euclidean_rhythm in enumerate(self.euclidean_rhythms):
                euclidean_rhythm.set_parameters_from_rhythm(
                    self.presets[self._load_preset_index][index])
                # in the case of no expander, we just disable the algo_custom
                if euclidean_rhythm.algo_custom:
                    euclidean_rhythm.algo_custom = False
                    # todo remove euclidean_rhythm.algo_index = LxEuclidConstant.ALGO_EUCLIDEAN
                euclidean_rhythm.set_rhythm()
    ...
    def update_all_gates_length_percentage_time_ms(self):
        for euclidean_rhythm in self.euclidean_rhythms:
            if self.clk_mode == LxEuclidConstant.TAP_MODE:
                euclidean_rhythm.update_gate_length_percentage_time_ms(
                    self.clk_mode, self.tap_delay_ms)
            else:
                euclidean_rhythm.update_gate_length_percentage_time_ms(
                    self.clk_mode, self.lx_hardware.clock_period_avg_tenth_ms)
        ...
        if event == LxEuclidConstant.EVENT_TAP_MENU_BTN_LONG:
            ...
            if event == LxEuclidConstant.EVENT_INNER_CIRCLE_TAP:
                ...
                if self.param_pads_page == 0:  # action
                    ...
                    if rotate_action_index not in [LxEuclidConstant.CIRCLE_ACTION_NONE, LxEuclidConstant.CIRCLE_ACTION_RESET, LxEuclidConstant.CIRCLE_ACTION_MUTE, LxEuclidConstant.CIRCLE_ACTION_FILL, LxEuclidConstant.CIRCLE_ACTION_BURST]:
            ...
            if event == LxEuclidConstant.EVENT_BTN_SWITCHES:
                ...
                angle_inner = self.lx_hardware.capacitives_circles.inner_circle_angle
                config_index = angle_to_index(angle_inner, 5)
            ...
            if event == LxEuclidConstant.EVENT_BTN_SWITCHES and data == self.sm_rhythm_param_counter:
                ...
                if self.param_channel_config_page == 0:  # CV
                    ...
                    previous_algo_custom = self.euclidean_rhythms[self.sm_rhythm_param_counter].algo_custom

                    if algo_selection == 4:  # Custom rhythm selection
                        self.euclidean_rhythms[self.sm_rhythm_param_counter].algo_custom = True
                        ...
                        self.lx_hardware.set_expander_rhythm_and_focus(
                            rhythm_copy, self.sm_rhythm_param_counter, self.euclidean_rhythms[self.sm_rhythm_param_counter].beats, self.euclidean_rhythms[self.sm_rhythm_param_counter].offset)
                    else:  # Regular algorithm selection (0-3)
                        ...
                        if previous_algo_custom:
                            ...
                            for rhythm_index in range(0, 4):
                                if self.euclidean_rhythms[rhythm_index].algo_custom:
                                    ...
                                    self.lx_hardware.set_expander_rhythm_and_focus(
                                        rhythm_copy, rhythm_index, self.euclidean_rhythms[rhythm_index].beats, self.euclidean_rhythms[rhythm_index].offset)
                                    rhythm_found = True
                                    break
                            if not rhythm_found:
                                self.lx_hardware.clear_expander_focus()
    ...
    def load_data(self):
        ...
        if self.v_minor is not eeprom_v_minor or self.v_major is not eeprom_v_major:
            ...
            if self.v_fix is not eeprom_v_fix:
                ...
                print(
                    "Info: main memory version number is the same as in eeprom", version_eeprom)
            ...
            try:
                ...
                for rhythm_index, euclidean_rhythm in enumerate(self.euclidean_rhythms):
                    ...
                    if self.lx_hardware.lx_pander_seq is not None:
                        ...
                        self.lx_hardware.set_expander_rhythm(
                            euclidean_rhythm.custom_rhythm.copy(), rhythm_index, euclidean_rhythm.beats, euclidean_rhythm.offset)

                    if euclidean_rhythm.algo_custom and not algo_4_rhythm_found:
                ...
                for euclidean_rhythm in self.euclidean_rhythms:
                    euclidean_rhythm.algo_custom = bool(
                        self.lx_hardware.get_eeprom_data_int(incr_addr(eeprom_addr)))
                ...
                for preset in self.presets:
                    for preset_euclidean_rhythm in preset:
                        preset_euclidean_rhythm.algo_custom = bool(
                            self.lx_hardware.get_eeprom_data_int(incr_addr(eeprom_addr)))
    ...
    def test_mode(self):
        ...
        quit_test_mode = False
        btn_released = False
        while not quit_test_mode:
            for i in range(0, 4):
                self.lx_hardware.sw_leds[i].value(
                    self.lx_hardware.btn_menu_pins[i].value())
            self.lx_hardware.led_menu.value(
                self.lx_hardware.btn_menu_pin.value())
            self.lx_hardware.led_tap.value(
                self.lx_hardware.btn_tap_pin.value())
            ...
            if (self.lx_hardware.btn_tap_pin.value() and self.lx_hardware.btn_menu_pin.value()) == 1:
                btn_released = True

            # detect button re-press to quit test mode
